...
const TagModal = ({ open, onClose, currentTag, reload, totalTags }) => {
  ...
  useEffect(() => {
    try {
      (async () => {
        ...
        const { data } = await api.get(`/tags/${currentTag.id}`);
  ...
  const handleSaveTag = async (values) => {
    const tagData = {
      ...
      userId: user.id,
      kanban,
      order: currentTag ? currentTag.order : totalTags + 1,
    };

    try {
      if (currentTag) {
        await api.put(`/tags/${currentTag.id}`, tagData);
      } else {
